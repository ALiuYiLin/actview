...
| 无 props 声明的组件 | 所有属性都是 attrs → 全部 fallthrough；setup 的 props 为空对象 | ⚠️ 函数形态保持「props 全量 + attrs 全量」（向后兼容策略 A） |
...
| 函数形态 props | 保持全量（向后兼容），attrs 同全量 —— 需要 props 白名单时才用 options 形态 |

...
describe('props 响应式（shallowReactive）', () => {
  ...
  it('props 是桥接 Proxy（v2）：props 自身是代理，嵌套对象不代理', () => {
    const plain = { deep: { x: 1 } }
    let captured: any
    function Child(props: any) {
      captured = props
...
describe('场景 27：props 全量 + 显式透传', () => {
  ...
  it('显式 {...props} 透传：class 合并落根（v2/vue mergeProps 语义）', () => {
    ...
    // v2：vue mergeProps 对 class 是合并（非 React 覆盖）——自带 + 外部共存
    expect(div.className).toContain('panel')
  ...
  it('key/ref 是 vue 协调属性：都不进 props（v2 语义）', () => {
    let captured: any
    let refVal: any = null
    ...
    // key 是协调键，永不进 props（React/Vue 一致）
    expect(captured.key).toBeUndefined()
    // v2：ref 是 vue 特殊属性（组件 ref = 组件实例，非 DOM），不进 props

...
import { createApp, createContext, reactive, ref } from 'actview'
...
describe('createContext（store-as-is）', () => {
  ...
  it('响应式通道③：ref 本体直传 → 消费端 .value 活读', async () => {
    ...
    function App() {
      // v2：JSX 无顶层 ref 解包——ref 本体直接传（等价 v1 rawRef 语义）
      return <CountCtx.Provider value={count}><Consumer /></CountCtx.Provider>
    }
    const host = mount(App)
    expect(host.querySelector('.v')!.textContent).toBe('0')
  ...
  // v1 的 SSR（renderToString）语义测试不迁移：v2 SSR 走 vue
  //（@vue/server-renderer），见 test/ssr 目录（v1 内部实现，不参与 v2）
})

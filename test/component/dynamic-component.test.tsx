// ============================================================
// 动态组件切换（v2：变量组件——React 语义；<component is> 是 v1 方言，
// 在 v2 编译成字符串 'component' 标签，vue 不解析）
// ============================================================

import { describe, it, expect } from 'vitest'
import { createApp, reactive, nextTick } from 'actview'
...
describe('场景 14：插槽与动态组件', () => {
  it('动态组件（变量组件）切换', async () => {
    ...
    function App() {
      // v2：动态组件用 JSX 内联三元（render 期求值）——组件函数体是
      // setup（只执行一次），变量组件在 setup 期求值会成快照
      return <div>{state.view === 'a' ? <A /> : <B />}</div>
    }

// ============================================================
// v-memo 指令运行时行为测试（拆分自 test/vmemo.test.tsx）
// 运行：pnpm exec vitest run test/renderer/v-memo.test.tsx
// ============================================================
...
describe('v-memo 指令：运行时', () => {
  it('局部更新：deps 未变行零 DOM 变更，deps 变化行正常更新', async () => {
    ...
    expect(mutations).toBe(0)                    // 未变行零 DOM 操作（短路）
  ...
  it('选中高亮：变化只影响受更新的行（其他行短路）', async () => {
    const rows = reactive([
      { id: 1, label: 'a' },
      { id: 2, label: 'b' },
      { id: 3, label: 'c' },

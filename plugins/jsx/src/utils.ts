...
// ============================================================
// React 语义别名（@actview/plugin-jsx 对齐层）
// 仅原生元素生效；组件 props 原样保留（组件可能声明 className/onChange prop）
// ============================================================

/** 静态判定 input[type=checkbox|radio]：这些元素的 React onChange = 原生 change */
export function inputTypeIsCheckOrRadio(
  props: NodePath<t.JSXAttribute | t.JSXSpreadAttribute>[],
): boolean {
  for (const attr of props) {
    if (!attr.isJSXAttribute()) continue
    const node = attr.node
    if (t.isJSXIdentifier(node.name) && node.name.name === 'type') {
      return (
        t.isStringLiteral(node.value) &&
        (node.value.value === 'checkbox' || node.value.value === 'radio')
      )
    }
  }
  return false
...
/**
 * React 属性名 → Vue 运行时属性名：
 *  - className → class（Vue patchProp 不认识 className）
 *  - htmlFor → for（JSX 保留字）
 *  - onChange → onInput（text-like input/textarea：React onChange = 原生 input 事件；
 *    checkbox/radio/select 的 React onChange 本就是原生 change，保留）
 */
export function applyReactPropAliases(
  name: string,
  isComponent: boolean,
  tag: t.Identifier | t.CallExpression | t.StringLiteral | t.MemberExpression,
  props: NodePath<t.JSXAttribute | t.JSXSpreadAttribute>[],
): string {
  ...
  switch (name) {
    ...
    case 'onChange': {
      ...
      if (tagName === 'textarea') return 'onInput'
      if (tagName === 'input' && !inputTypeIsCheckOrRadio(props)) return 'onInput'
      return name
    }
    default:
      return name

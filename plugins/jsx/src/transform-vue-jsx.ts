...
function buildProps(path: NodePath<t.JSXElement>, state: State) {
  ...
  props.forEach((prop) => {
    if (prop.isJSXAttribute()) {
      ...
      let attributeValue = getJSXAttributeValue(prop, state)
      ...
      if (!isComponent && name === 'dangerouslySetInnerHTML') {
        name = 'innerHTML'
        if (!t.isObjectExpression(attributeValue)) {
          throw path.buildCodeFrameError(
            '@actview/plugin-jsx: dangerouslySetInnerHTML 必须传 { __html: ... } 对象',
          )
        }
        const htmlProp = attributeValue.properties.find(
        ...
        if (!htmlProp) {
          throw path.buildCodeFrameError(
            '@actview/plugin-jsx: dangerouslySetInnerHTML 必须包含 __html 字段',
          )
        }
        attributeValue = htmlProp.value as t.Expression
      }

// ============================================================
// CSS 变换 — 移植 Vue 3 compiler-sfc 的 scoped PostCSS 插件
// 选择器末尾追加 [data-v-<hash>]；支持 :deep()/:slotted()/:global()；
// keyframes 重命名并同步改写 animation-name 引用；>>> /deep/ 兼容
// ============================================================
...
/** 生成 scoped hash：md5(文件绝对路径).slice(0,8)（与 React 插件 vite-plugin-scoped-css 约定一致） */
export function getHash(absPath: string): string {
  return crypto.createHash('md5').update(absPath).digest('hex').slice(0, 8)
}

/** 完整 scoped 属性名：data-v-<hash>（attrPrefix 支持自定义前缀） */
export function scopeAttr(hash: string, attrPrefix = 'data-v'): string {
  return `${attrPrefix}-${hash}`
}
...
function warn(msg: string) {
  console.warn(`[plugin-scoped] ${msg}`)
}
...
const scopedPlugin: PluginCreator<string> = (id = '') => {
  ...
  return {
    postcssPlugin: 'actview-sfc-scoped',
    Rule(rule) {
      processRule(id, rule)
    },
    AtRule(node) {
...
/** 纯函数：对 CSS 应用 scoped 变换，返回变换后的 CSS（插件实例每次新建，keyframes 状态隔离） */
export async function transformScopedCSS(
  css: string,
  hash: string,
  attrPrefix = 'data-v',
): Promise<string> {
  const id = `${attrPrefix}-${hash}`
  // postcss 8.5+ 移除了同步 processSync，使用 async process
  const result = await postcss([scopedPlugin(id)]).process(css, {
    from: undefined,
  })
  return result.css
}

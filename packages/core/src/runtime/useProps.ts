...
/**
 * 单个 prop 的响应式取值。normalize 缺省时等价于 `toRef(props, key)`
 * （原始值，不做默认值处理）。
 */
export function useProp<T = any>(
  ...
  return computed(() => (normalize ? normalize(props[key]) : props[key]))
}
 ...
 */
export function useProps<M extends Record<string, Normalizer>>(
  props: Record<string, any>,
  map: M
): { [K in keyof M]: ComputedRef<ReturnType<M[K]>> } & {
  rest: ComputedRef<Record<string, any>>
} {
  const named: { [K in keyof M]: ComputedRef<ReturnType<M[K]>> } = {} as any
  for (const key in map) {
    const fn = map[key]
    named[key] = computed(() => (fn ? fn(props[key]) : props[key]))
  }
  const rest = computed(() => {
